...
- Files: `docs/superpowers/specs/2026-06-24-ai-food-mvp-design.md`, `src/features/add-food/ui/ImagePicker.tsx`, `src/features/analyze-food/model/useAnalyzeFood.ts`, `src/shared/api/client.ts`, `src/app/router.tsx`, `src/pages/home/ui/HomePage.tsx`
...
- Issue: Diary state lives in `src/entities/meal/model/useDiaryStore.ts` (correct FSD placement), but `docs/superpowers/plans/2026-06-24-ai-food-mvp.md` still references `src/features/save-meal/model/useDiaryStore.ts`. The `save-meal` feature now only exports `useSaveMeal` from `src/features/save-meal/index.ts`.
- Files: `src/entities/meal/model/useDiaryStore.ts`, `src/features/save-meal/model/useSaveMeal.ts`, `docs/superpowers/plans/2026-06-24-ai-food-mvp.md`
...
- Files: `src/widgets/daily-header/ui/DailyHeader.tsx`, `src/widgets/nutrition-card/ui/NutritionCard.tsx`, `src/features/save-meal/model/useSaveMeal.ts`
...
- Files: `src/entities/meal/model/useDiaryStore.ts`, `src/entities/meal/model/useDiaryStore.test.ts`, `src/pages/diary/ui/DiaryPage.tsx`
...
- Issue: `<Toaster />` is rendered in `src/app/providers.tsx`, but no `toast()` calls exist anywhere in ``.
- Files: `src/app/providers.tsx`, `package.json`
...
- Issue: `ReactQueryDevtools` is unconditionally imported in `src/app/providers.tsx`.
...
- Files: root `package.json`, `package.json`, `[removed-backend]/package.json`
- Impact: Style and import-rule violations (e.g. `.cursor/rules/index-reexports.mdc`) are not enforced; drift accumulates silently.
- Fix approach: Add ESLint + Prettier (or Biome) at repo root with FSD/import rules.
...
- Files: `.cursor/rules/index-reexports.mdc`, all cross-slice imports under `src/`
...
- Files: `src/shared/types/index.ts`, `src/features/save-meal/model/useSaveMeal.ts`, `src/widgets/nutrition-card/ui/NutritionCard.tsx`
...
- Files: `src/features/analyze-food/api/analyzeFoodApi.ts`
...
- Files: `src/shared/api/client.ts`, `src/features/analyze-food/model/useAnalyzeFood.ts`, `src/pages/result/ui/ResultPage.tsx`
...
- Files: `[removed-backend]/src/routes/analyze-food.ts`
...
- Risk: `app.use(cors())` in `[removed-backend]/src/index.ts` allows any origin — acceptable for local mock, unsafe if deployed publicly.
- Files: `[removed-backend]/src/index.ts`
...
- Files: `src/features/add-food/ui/ImagePicker.tsx`, `src/features/analyze-food/api/analyzeFoodApi.ts`
- Current mitigation: None.
...
- Files: `src/features/analyze-food/api/analyzeFoodApi.ts`, `src/features/add-food/model/useImageStore.ts`
...
- Problem: Hook sets `staleTime: 0`, overriding global `30_000` in `src/shared/lib/queryClient.ts`.
- Files: `src/features/analyze-food/model/useAnalyzeFood.ts`, `src/shared/lib/queryClient.ts`
...
- Files: `src/entities/meal/model/useDiaryStore.ts`
...
- Files: `src/pages/result/ui/ResultPage.tsx`
...
- Files: `src/widgets/daily-header/ui/DailyHeader.tsx`, `src/widgets/meal-list/ui/MealList.tsx`
...
- Files: `src/features/save-meal/model/useSaveMeal.ts` (imports `@/features/add-food` and `@/entities/meal`)
...
- Files: `src/entities/meal/model/useDiaryStore.ts`
- Why fragile: Changing `Meal` shape in `src/shared/types/index.ts` can corrupt or crash hydrated state.
...
- Files: `src/features/analyze-food/model/useAnalyzeFood.ts`
...
- Current capacity: One hardcoded `MOCK_RESPONSE` in `[removed-backend]/src/routes/analyze-food.ts`.
...
- Files: `[removed-backend]/src/index.ts`, `[removed-backend]/src/routes/analyze-food.ts`
...
- Files: `src/features/save-meal/model/useSaveMeal.ts`
...
- Files: `src/shared/api/client.ts`, `src/features/analyze-food/api/analyzeFoodApi.ts`
