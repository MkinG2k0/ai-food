...
export const COMPOSITION_PROMPT_RULE =
  'Состав (items): составные/слойные блюда (бургер, сэндвич, ролл, шаурма, пицца с начинкой, салат-сборка) всегда разбивай на видимые ингредиенты/слои. Пример: бургер → отдельные item «Булка», «Котлета», «Сыр», «Салат», «Помидор» — не оставляй один item «Гамбургер»/«Бургер», когда на фото видны слои. Простые однородные продукты (картофель фри, яблоко, стакан сока) — один item допустим. foodName = название всего приёма; item/name = атомарные компоненты — не дублируй название составного блюда как единственный item, если видны части.';

/**
 * itemCount = separate servings the user would count, NOT pieces inside one plated dish.
 * Fixes: chicken nuggets/pieces on one plate → 1, not 6.
 */
export const ITEM_COUNT_PROMPT_RULE = `itemCount — число отдельных ПОРЦИЙ (сколько целых «подач» на фото), а НЕ кусков внутри одного блюда.
itemCount = 1, если одна тарелка/миска/контейнер с одним блюдом — даже когда внутри много кусков: курица/наггетсы в соусе, мясо кусочками, салат, паста, рагу, гарнир дольками. Пример: тарелка курицы в панировке из 5–6 кусков с овощами → itemCount=1 (не 6).
itemCount > 1 только если видно несколько отдельно взятых одинаковых единиц, которые обычно считают поштучно как целые порции: 2 ролла суши, 2 бургера, 3 отдельных печенья без общего «блюда с соусом».
НЕ считай: куски мяса/рыбы в одном соусе на одной тарелке, дольки, листья, кусочки гарнира, ингредиенты состава (это items, не itemCount).
НЕ равно длине массива items. КБЖУ всегда на всё видимое блюдо целиком.`;
...
const GEMINI_COMPOSITION_PROMPT_RULE =
  'Составные/слойные блюда (бургеры, салаты, супы, боулы) всегда разбивай на видимые ингредиенты/слои, не оставляй их одной строкой.';
...
- Упаковка/этикетка продукта без видимой порции еды → noFood.
