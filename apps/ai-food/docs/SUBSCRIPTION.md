...
1. Гость: бесплатные AI (`analyze` + `refine`) по `X-Device-Id` (лимит в админке, default 50). Отдельная галочка согласия в UI для гостя **не** показывается: в Политике конфиденциальности (`apps/ai-web` → `/privacy`) это основание сформулировано как предоставление услуги / законный интерес оператора для квоты и антиабьюза — перед продом проверить с юристом.
2. Лимит → `/login` (Telegram)
3. После входа: бонус к квоте **профиля** (админка, default +100 → итого **150**); гостевые события текущего устройства привязываются к `userId`
4. Логин **сам по себе не даёт** unlimited
5. Без лицензии — квота по `userId` (общая на все устройства); после исчерпания → `/subscribe`
...
Синхронизация аккаунта после входа (дневник / профиль / вес / избранное / настройки) **не** гейтится подпиской. **Фото приёмов** на сервер не уходят и **не будут** — только на устройстве. Канон: [`USER-DATA-SYNC.md`](./USER-DATA-SYNC.md).
