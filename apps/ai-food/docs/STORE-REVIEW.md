...
1. ~~Политика: явная фраза про «фото не храним на сервере (только транзитный analyze)»~~ — **готово** (`apps/ai-web/src/lib/legal/privacyContent.ts`, дата редакции 2026-08-24). Гостевая квота по `deviceId` без отдельной галочки — в §§2–4 политики (услуга / законный интерес; проверка юристом перед продом).
