...
**Главное для продукта:** после входа восстанавливаются профиль (вкл. микро), дневник, избранное, вес, настройки.

### Фото приёмов — locked product decision

**Фото еды никогда не загружаются и не хранятся на сервере** (нет blob storage, нет sync файлов). В API уходят только URI-stubs; байты остаются в локальном Filesystem.

**Почему:** конфиденциальность — снимки тарелки / кухни / окружения считаем чувствительными; не хотим централизованного хранилища фото пользователей. Это не временный пробел и не «потом добавим S3».

Следствие UX: на новом устройстве дневник/КБЖУ восстанавливаются, превью-фото — нет (нужно снимать заново или жить без картинки).
