...
/**
 * Wait until LatestNewsSheet is dismissed or confirmed absent after async
 * news-store hydration (Capacitor Preferences rehydration).
 */
export async function waitForNewsSheetSettled(page: Page): Promise<void> {
  const gotIt = page.getByRole('button', { name: 'Понятно' });
  const settleMs = 200;
...
/** Close remaining bottom sheets (streak celebration, etc.) that block clicks. */
...
export async function waitForHome(page: Page): Promise<void> {
  ...
  await waitForNewsSheetSettled(page);
