...
describe('mealFoodTypeUi', () => {
  it('maps every food type to a Russian theme-aware Lucide thumbnail', () => {
    for (const foodType of FOOD_TYPES) {
      ...
      expect(ui?.tileClass).toMatch(/^bg-\w+-500\/15$/);
      expect(ui?.iconClass).toMatch(/^text-\w+-700 dark:text-\w+-300$/);
    }
  });
