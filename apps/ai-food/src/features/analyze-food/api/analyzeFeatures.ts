/** Feature flags that shape analyze/refine prompts and UI. */
...
export function applyAnalyzeFeaturesToPrompt(
  ...
  if (!features.healthiness) {
    result = result
      ...
      .replace(
        /[ \t]*"healthiness"\s*:\s*number[^,\n]*,?\n?/gi,
        '',
      )
      .replace(
        /[ \t]*"healthinessReason"\s*:\s*string[^,\n]*,?\n?/gi,
        '',
      );
  }
...
/** Drop fields the user disabled so they are not persisted. */
export function maskNutritionResultByFeatures<T extends {
  healthiness?: number;
  healthinessReason?: string;
  micronutrients?: unknown;
}>(result: T, features: AnalyzeFeatures): T {
  const next = { ...result };
  if (!features.healthiness) {
    delete next.healthiness;
    delete next.healthinessReason;
  }
  if (!features.vitamins) {
    delete next.micronutrients;
  }
  return next;
}
