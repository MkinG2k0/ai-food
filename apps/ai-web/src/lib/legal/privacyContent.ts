  ...
  return [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        'Для AI-анализа: фото еды и текстовые описания передаются в AI Gateway на время обработки запроса (см. раздел 5). Сами фото оператором не сохраняются и не хранятся после обработки.',
    ...
    },
    {
      ...
      paragraphs: [
        'При анализе фото еды изображение и сопроводительный текст передаются в AI Gateway и далее провайдерам моделей (в том числе через OpenRouter) исключительно для возврата результата анализа. Сами фото оператором не сохраняются: они используются только на время запроса и не хранятся после обработки.',
    ...
    },
    {
      ...
      paragraphs: [
        ...
        'Фото для анализа обрабатываются только в рамках запроса и оператором не сохраняются; долговременный архив фотографий еды не ведётся.',
